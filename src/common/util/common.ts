...
export const resetPasswordCheck=(password:string, confirmPassword:string):boolean=>{
    if(password.trim()==='' || password.trim().length===0){
        return false;
    }
    else if(password.trim().length<8 || password.trim().length>30){
    ...
    else if(confirmPassword.trim().length<8 || confirmPassword.trim().length>30){
        return false;
    }
...
export const changePasswordCheck=(oldPassword:string, newPassword:string,
